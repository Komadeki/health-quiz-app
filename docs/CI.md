...
`check_health.sh`, and `check_fixture.sh` in `tooling/ci/`.
...
| Change | Checks |
| --- | --- |
| `packages/**`, `tooling/**`, `question_banks/**`, `.github/**`, shared root config | shared + health + fixture |
...
| unknown path | shared + health + fixture |

Mixed health and fixture changes run both app jobs. An empty or unclassified
...
non-fatal lint policy and fixture's strict analysis policy.
...
`flutter build ios --no-codesign` for `apps/health` and
`apps/_single_unlock_fixture`. It performs no signing, upload, archive delivery,
or App Store action.
