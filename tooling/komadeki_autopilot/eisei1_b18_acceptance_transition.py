...
BATCH=REPO/"question_banks/eisei1/authoring/batches/batch_018"; STATE=REPO/"tooling/komadeki_autopilot/eisei1_state.json"
IDS=("E1-B18-HG-C001",)
st=json.loads(STATE.read_text(encoding="utf-8"))
if st.get("state_epoch")!=38 or st.get("next_atomic_objective")!="ACCEPT_EISEI1_B18_CANDIDATES_1": raise SystemExit("unexpected state")
...
if set(by)!=set(IDS) or any(r["state"]!="AI_PRE_ACCEPT" for r in rows) or set(dec)!=set(IDS) or any(x["decision"]!="ACCEPT" for x in dec.values()): raise SystemExit("B18 preaccept/review drift")
...
st.update(observed_main=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),next_atomic_objective="ALLOCATE_AND_INTEGRATE_EISEI1_B18_ACCEPTED_1",state_epoch=39); STATE.write_text(json.dumps(st,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
