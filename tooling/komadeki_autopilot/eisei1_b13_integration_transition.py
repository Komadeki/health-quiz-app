...
B=R/"question_banks/eisei1"; A=B/"authoring"; X=A/"batches/batch_013"; S=R/"tooling/komadeki_autopilot/eisei1_state.json"
IDS=("E1-B13-LH-C001","E1-B13-LH-C002"); EXPECTED={c:f"EISEI1-Q-{n:06d}" for c,n in zip(IDS,range(28,30))}
def read(p,k):
...
if st.get("state_epoch")!=23 or st.get("next_atomic_objective")!="ALLOCATE_AND_INTEGRATE_EISEI1_B13_ACCEPTED_2":raise SystemExit("unexpected state")
if set(before)!={f"EISEI1-Q-{n:06d}" for n in range(1,28)} or any(candidates[c]["state"]!="READY_FOR_ID" or candidates[c].get("permanent_question_id") for c in IDS):raise SystemExit("precondition drift")
...
if set(read(A/"questions.csv","question_id"))!={f"EISEI1-Q-{n:06d}" for n in range(1,30)}:raise SystemExit("inventory mismatch")
...
st.update(observed_main=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),next_atomic_objective="VERIFY_EISEI1_B13_CANONICAL_SOURCES_2",state_epoch=24);S.write_text(json.dumps(st,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
