...
if state.get("state_epoch") != 32 or state.get("next_atomic_objective") != "VERIFY_OTSU4_BATCH_2_CANONICAL_SOURCES_23":
...
if any(qid in verified_ids for qid in EXPECTED.values()):
    raise SystemExit("partial Otsu4 Batch 2 source verification detected")
...
if len(verifications) != 29 or len({r["question_id"] for r in verifications}) != 29:
...
if any(after[c]["state"] != "VERIFIED" or after[c]["permanent_question_id"] != EXPECTED[c] for c in ACCEPTED):
    raise SystemExit("Otsu4 Batch 2 verified state mismatch")
if any(after[c]["state"] != "AI_PRE_ACCEPT" or after[c]["permanent_question_id"] for c in REJECTED):
    raise SystemExit("rejected Otsu4 Batch 2 candidate changed during verification")
...
state["state_epoch"] = 33
state["next_atomic_objective"] = "PLAN_OTSU4_BATCH_3_600Q_COVERAGE_BATCH"
