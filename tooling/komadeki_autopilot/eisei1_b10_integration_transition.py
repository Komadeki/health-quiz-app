#!/usr/bin/env python3
"""Integrate the three accepted Eisei1 B10 candidates without release mutation."""
...
SELECTED = ("E1-B10-LH-C001", "E1-B10-LH-C002", "E1-B10-LH-C003")
EXPECTED = {
    "E1-B10-LH-C001": "EISEI1-Q-000017",
    "E1-B10-LH-C002": "EISEI1-Q-000018",
    "E1-B10-LH-C003": "EISEI1-Q-000019",
}
...
if state.get("state_epoch") != 8 or state.get("next_atomic_objective") != "ALLOCATE_AND_INTEGRATE_EISEI1_B10_ACCEPTED_3":
...
if set(questions_before) != {f"EISEI1-Q-{number:06d}" for number in range(1, 17)}:
    raise SystemExit("canonical inventory drift before B10 integration")
if set(registry_before) != set(questions_before):
    raise SystemExit("registry drift before B10 integration")
if any(candidates[candidate_id]["state"] != "READY_FOR_ID" or candidates[candidate_id].get("permanent_question_id") for candidate_id in SELECTED):
    raise SystemExit("B10 candidates must be READY_FOR_ID with no permanent ID")
if {path.stem for path in (BATCH / "acceptance_packets").glob("*.json")} != set(SELECTED):
    raise SystemExit("B10 acceptance packet set drift")
...
if transaction.plan().mapping != EXPECTED or transaction.apply() != EXPECTED:
    raise SystemExit("unexpected B10 permanent-ID allocation")
...
candidates_after = read_rows(BATCH / "candidates.csv", "candidate_id")
if set(questions_after) != set(questions_before) | set(EXPECTED.values()):
    raise SystemExit("B10 canonical inventory mismatch")
if set(registry_after) != set(questions_after):
    raise SystemExit("B10 registry inventory mismatch")
for candidate_id, question_id in EXPECTED.items():
    candidate = candidates_after[candidate_id]
    if candidate["state"] != "INTEGRATED" or candidate["permanent_question_id"] != question_id:
        raise SystemExit(f"B10 candidate lifecycle mismatch: {candidate_id}")
    if registry_after[question_id]["notes"] != f"Expansion pre-release allocation: {candidate_id}":
        raise SystemExit(f"B10 registry provenance mismatch: {question_id}")
if any(path.read_bytes() != before for path, before in untouched.items()):
    raise SystemExit("B10 integration mutated out-of-scope artifact")
...
state["next_atomic_objective"] = "VERIFY_EISEI1_B10_CANONICAL_SOURCES_3"
state["state_epoch"] = 9
