...
AUTHOR_ID = "otsu4-b3-author-r1"
REVIEWER_ID = "otsu4-b3-independent-reviewer-r1"
DIRECTOR_ID = "otsu4-b3-director-adjudicator-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 37 or state.get("next_atomic_objective") != "MATERIALIZE_OTSU4_BATCH_3_ACCEPTANCE_PACKETS_22":
...
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ALL):
    raise SystemExit("unexpected Otsu4 Batch 3 pre-acceptance state")
...
if review.get("summary") != {"reviewed": 24, "accept": 22, "reject": 2, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected Otsu4 Batch 3 review summary")
if set(review_by) != set(ALL):
    raise SystemExit("Otsu4 Batch 3 review candidate set drift")
if any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED):
    raise SystemExit("Otsu4 Batch 3 accepted review decision drift")
if any(review_by[c].get("decision") != "REJECT" for c in REJECTED):
    raise SystemExit("Otsu4 Batch 3 rejected review decision drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("Otsu4 Batch 3 review identity drift")

director = json.loads((BATCH / "director_adjudication_r1.json").read_text(encoding="utf-8"))
if director.get("summary") != {"accept": 22, "reject": 2, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected Otsu4 Batch 3 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ACCEPTED):
    raise SystemExit("Otsu4 Batch 3 Director accepted set drift")
...
if set(rejected_by) != set(REJECTED):
    raise SystemExit("Otsu4 Batch 3 Director rejected set drift")
if director.get("identity_separation") != "PASS" or director.get("author", {}).get("id") != AUTHOR_ID or director.get("reviewer", {}).get("id") != REVIEWER_ID or director.get("director", {}).get("id") != DIRECTOR_ID:
    raise SystemExit("Otsu4 Batch 3 Director actor identity drift")
if len({AUTHOR_ID, REVIEWER_ID, DIRECTOR_ID}) != 3:
    raise SystemExit("Otsu4 Batch 3 actor identity collision")
director_rationale = str(director.get("director_rationale", "")).strip()
if not director_rationale:
    raise SystemExit("Otsu4 Batch 3 Director rationale missing")

packets = BATCH / "acceptance_packets"
packets.mkdir(exist_ok=True)
if list(packets.glob("*.json")):
    raise SystemExit("partial Otsu4 Batch 3 acceptance packet state detected")
...
if {p.stem for p in packets.glob("*.json")} != set(ACCEPTED):
    raise SystemExit("Otsu4 Batch 3 packet set drift")
for cid in ACCEPTED:
...
if any((packets / f"{cid}.json").exists() for cid in REJECTED):
    raise SystemExit("rejected Otsu4 Batch 3 candidate received acceptance packet")
...
if len(canonical_questions) != 29 or len(registry) != 29 or len(verifications) != 29 or released:
    raise SystemExit("Otsu4 canonical/release baseline changed before Batch 3 acceptance")
...
state["next_atomic_objective"] = "PROMOTE_OTSU4_BATCH_3_ACCEPTED_22_TO_READY_FOR_ID"
