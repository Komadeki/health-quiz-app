...
ACCEPTED = tuple(f"B15-RISK-C{i:03d}" for i in range(1, 10))
MAPPING = {cid: f"DRONE-Q-{n:06d}" for cid, n in zip(ACCEPTED, range(346, 355))}

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("next_atomic_objective") != "VERIFY_B15_CANONICAL_SOURCES_9" or state.get("state_epoch") != 107:
...
for cid, qid in MAPPING.items():
    ...
    if not r["source_locator"].startswith("教則 第6章") or "〔一等〕" in r["source_locator"]:
        raise SystemExit(f"source locator/scope mismatch: {cid}")

questions = {r["question_id"]: r for r in read_csv(AUTHORING / "questions.csv")}
if len(questions) != 354:
    raise SystemExit(f"canonical inventory expected 354, got {len(questions)}")
for cid, qid in MAPPING.items():
...
if any(qid in existing for qid in MAPPING.values()):
    raise SystemExit("partial/duplicate B15 source verification detected")
...
state["next_atomic_objective"] = "PLAN_B16_400_TARGET_COVERAGE_VARIANTS"
state["state_epoch"] = 108
