...
B=R/"question_banks/eisei1"; A=B/"authoring"; X=A/"batches/batch_019"; S=R/"tooling/komadeki_autopilot/eisei1_state.json"
IDS=("E1-B19-LH-C001",); EXPECTED={IDS[0]:"EISEI1-Q-000035"}
def read(p,k):
...
if st.get("state_epoch")!=42 or st.get("next_atomic_objective")!="ALLOCATE_AND_INTEGRATE_EISEI1_B19_ACCEPTED_1":raise SystemExit("unexpected state")
if set(before)!={f"EISEI1-Q-{n:06d}" for n in range(1,35)} or any(candidates[c]["state"]!="READY_FOR_ID" or candidates[c].get("permanent_question_id") for c in IDS):raise SystemExit("precondition drift")
...
if set(read(A/"questions.csv","question_id"))!={f"EISEI1-Q-{n:06d}" for n in range(1,36)}:raise SystemExit("inventory mismatch")
...
st.update(observed_main=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),next_atomic_objective="VERIFY_EISEI1_B19_CANONICAL_SOURCES_1",state_epoch=43);S.write_text(json.dumps(st,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
