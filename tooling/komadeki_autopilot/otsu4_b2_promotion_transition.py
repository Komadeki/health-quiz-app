...
if state.get("state_epoch") != 30 or state.get("next_atomic_objective") != "PROMOTE_OTSU4_BATCH_2_ACCEPTED_23_TO_READY_FOR_ID":
...
if any(after[c]["state"] != "READY_FOR_ID" or after[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("accepted Otsu4 Batch 2 promotion failed")
...
if len(questions) != 6 or len(registry) != 6 or len(verifications) != 6 or released:
    raise SystemExit("canonical/release baseline changed during promotion")

state["observed_main"] = subprocess.check_output(["git", "rev-parse", "origin/main"], text=True).strip()
state["state_epoch"] = 31
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_OTSU4_BATCH_2_ACCEPTED_23"
