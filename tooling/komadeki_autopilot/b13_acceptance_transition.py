...
BATCH = AUTHORING / "batches" / "batch_013"
STATE_PATH = REPO / "tooling" / "komadeki_autopilot" / "drone_state.json"
ALL = tuple(f"B13-SYS-C{i:03d}" for i in range(1, 21))
ACCEPTED = ("B13-SYS-C002", "B13-SYS-C009", "B13-SYS-C014", "B13-SYS-C020")
REJECTED = tuple(cid for cid in ALL if cid not in ACCEPTED)
AUTHOR_ID = "chatgpt-b13-systems-author-r1"
REVIEWER_ID = "autopilot-b13-systems-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b13-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 91 or state.get("next_atomic_objective") != "MATERIALIZE_B13_ACCEPTANCE_PACKETS_4":
...
if set(rows) != set(ALL):
    raise SystemExit("unexpected B13 candidate set")
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ALL):
    raise SystemExit("unexpected B13 pre-acceptance state")
...
if review.get("summary") != {"reviewed":20,"accept":4,"reject":16,"rework":0,"hold":0}:
    raise SystemExit("unexpected B13 review summary")
if set(review_by) != set(ALL):
    raise SystemExit("B13 review candidate set drift")
if any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED) or any(review_by[c].get("decision") != "REJECT" for c in REJECTED):
    raise SystemExit("B13 review decision set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B13 review identity drift")

director = json.loads((BATCH / "director_adjudication_r1.json").read_text(encoding="utf-8"))
if director.get("summary") != {"accept":4,"reject":16,"rework":0,"hold":0}:
    raise SystemExit("unexpected B13 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ACCEPTED) or set(director.get("rejected_candidate_ids", [])) != set(REJECTED):
    raise SystemExit("B13 Director decision set drift")
...
state["state_epoch"] = 92
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B13_ACCEPTED_4"
STATE_PATH.write_text(json.dumps(state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
errors = validate_expansion_batch(BATCH)
if errors:
    raise SystemExit("B13 expansion validation failed: " + " | ".join(errors))
