...
BATCH = AUTHORING / "batches" / "batch_012"
STATE_PATH = REPO / "tooling" / "komadeki_autopilot" / "drone_state.json"
ALL = tuple(f"B12-OPS-C{i:03d}" for i in range(1, 21))
REJECTED = ("B12-OPS-C004",)
ACCEPTED = tuple(cid for cid in ALL if cid not in REJECTED)
AUTHOR_ID = "chatgpt-b12-operations-author-r1"
REVIEWER_ID = "autopilot-b12-operations-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b12-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 84 or state.get("next_atomic_objective") != "MATERIALIZE_B12_ACCEPTANCE_PACKETS_19":
...
if set(rows) != set(ALL):
    raise SystemExit("unexpected B12 candidate set")
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ALL):
    raise SystemExit("unexpected B12 pre-acceptance state")
...
if review.get("summary") != {"reviewed":20,"accept":19,"reject":1,"rework":0,"hold":0}:
    raise SystemExit("unexpected B12 review summary")
if set(review_by) != set(ALL):
    raise SystemExit("B12 review candidate set drift")
if any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED) or review_by[REJECTED[0]].get("decision") != "REJECT":
    raise SystemExit("B12 review decision set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B12 review identity drift")

director = json.loads((BATCH / "director_adjudication_r1.json").read_text(encoding="utf-8"))
if director.get("summary") != {"accept":19,"reject":1,"rework":0,"hold":0}:
    raise SystemExit("unexpected B12 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ACCEPTED) or set(director.get("rejected_candidate_ids", [])) != set(REJECTED):
    raise SystemExit("B12 Director decision set drift")
...
if any(after[c]["state"] != "READY_FOR_ID" or after[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("B12 accepted promotion failed")
if after[REJECTED[0]]["state"] != "AI_PRE_ACCEPT" or after[REJECTED[0]]["permanent_question_id"]:
    raise SystemExit("B12 rejected candidate mutated")
if {p.stem for p in packets.glob("*.json")} != set(ACCEPTED):
    raise SystemExit("B12 packet set drift")
...
state["state_epoch"] = 85
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B12_ACCEPTED_19"
STATE_PATH.write_text(json.dumps(state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
errors = validate_expansion_batch(BATCH)
if errors:
    raise SystemExit("B12 expansion validation failed: " + " | ".join(errors))
