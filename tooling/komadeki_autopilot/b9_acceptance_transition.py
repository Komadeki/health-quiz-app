...
ACCEPTED = tuple(f"B9-RISK-C{i:03d}" for i in range(1, 21))
AUTHOR_ID = "chatgpt-b9-risk-author-r1"
REVIEWER_ID = "autopilot-b9-risk-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b9-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 63 or state.get("next_atomic_objective") != "MATERIALIZE_B9_ACCEPTANCE_PACKETS_20":
...
if set(rows) != set(ACCEPTED):
    raise SystemExit("unexpected B9 candidate set")
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ACCEPTED):
    raise SystemExit("unexpected B9 pre-acceptance state")
...
if review.get("summary") != {"reviewed": 20, "accept": 20, "reject": 0, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected B9 review summary")
if set(review_by) != set(ACCEPTED) or any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED):
    raise SystemExit("B9 review accepted set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B9 review identity drift")

director = json.loads((BATCH / "director_adjudication_r1.json").read_text(encoding="utf-8"))
if director.get("summary") != {"accept": 20, "reject": 0, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected B9 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ACCEPTED) or director.get("identity_separation_checked") is not True:
    raise SystemExit("B9 Director accepted set/identity drift")
...
if any(after[c]["state"] != "READY_FOR_ID" or after[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("B9 promotion failed")
if {p.stem for p in packets.glob("*.json")} != set(ACCEPTED):
    raise SystemExit("B9 packet set drift")
...
state["state_epoch"] = 64
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B9_ACCEPTED_20"
