...
R=Path(__file__).resolve().parents[2];B=R/"question_banks/eisei1";A=B/"authoring";X=A/"batches/batch_015";S=R/"tooling/komadeki_autopilot/eisei1_state.json"
V={"EISEI1-Q-000031":"E1-MHLW-OH"}; ver="current-as-of-2026-08-26"
st=json.loads(S.read_text(encoding="utf-8"))
if st.get("state_epoch")!=31 or st.get("next_atomic_objective")!="VERIFY_EISEI1_B15_CANONICAL_SOURCES_1":raise SystemExit("unexpected state")
...
st.update(observed_main=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),next_atomic_objective="PLAN_EISEI1_NEXT_COVERAGE_WAVE_007",state_epoch=32);S.write_text(json.dumps(st,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
