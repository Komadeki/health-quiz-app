...
ALL = tuple(f"B11-RISK-C{i:03d}" for i in range(1, 21))
REJECTED = ("B11-RISK-C004", "B11-RISK-C010")
ACCEPTED = tuple(cid for cid in ALL if cid not in REJECTED)
AUTHOR_ID = "chatgpt-b11-risk-author-r1"
REVIEWER_ID = "autopilot-b11-risk-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b11-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 77 or state.get("next_atomic_objective") != "MATERIALIZE_B11_ACCEPTANCE_PACKETS_18":
...
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ALL):
    raise SystemExit("unexpected B11 pre-acceptance state")
...
expected_review_summary = {"reviewed": 20, "accept": 18, "reject": 2, "rework": 0, "hold": 0}
if review.get("summary") != expected_review_summary:
    raise SystemExit("unexpected B11 review summary")
if set(review_by) != set(ALL):
    raise SystemExit("B11 review candidate set drift")
if any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED):
    raise SystemExit("B11 accepted review set drift")
if any(review_by[c].get("decision") != "REJECT" for c in REJECTED):
    raise SystemExit("B11 rejected review set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B11 review identity drift")

director = json.loads((BATCH / "director_adjudication_r1.json").read_text(encoding="utf-8"))
if director.get("summary") != {"accept": 18, "reject": 2, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected B11 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ACCEPTED) or set(director.get("rejected_candidate_ids", [])) != set(REJECTED) or director.get("identity_separation_checked") is not True:
    raise SystemExit("B11 Director accepted/rejected set drift")
...
if any(after[c]["state"] != "READY_FOR_ID" or after[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("B11 accepted promotion failed")
if any(after[c]["state"] != "AI_PRE_ACCEPT" or after[c]["permanent_question_id"] for c in REJECTED):
    raise SystemExit("B11 rejected candidates mutated")
...
state["state_epoch"] = 78
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B11_ACCEPTED_18"
