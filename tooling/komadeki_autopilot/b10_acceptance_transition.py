...
ACCEPTED = tuple(f"B10-RULE-C{i:03d}" for i in range(1, 21))
AUTHOR_ID = "chatgpt-b10-rules-author-r1"
REVIEWER_ID = "autopilot-b10-rules-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b10-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 70 or state.get("next_atomic_objective") != "MATERIALIZE_B10_ACCEPTANCE_PACKETS_20":
...
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ACCEPTED):
    raise SystemExit("unexpected B10 pre-acceptance state")
...
if review.get("summary") != {"reviewed": 20, "accept": 20, "reject": 0, "rework": 0, "hold": 0}:
    raise SystemExit("unexpected B10 review summary")
if set(review_by) != set(ACCEPTED) or any(review_by[c].get("decision") != "ACCEPT" for c in ACCEPTED):
    raise SystemExit("B10 review accepted set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B10 review identity drift")
...
if {p.stem for p in packets.glob("*.json")} != set(ACCEPTED):
    raise SystemExit("B10 packet set drift")
...
state["state_epoch"] = 71
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B10_ACCEPTED_20"
