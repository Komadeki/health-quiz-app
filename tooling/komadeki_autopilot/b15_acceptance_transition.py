...
BATCH = AUTHORING / "batches" / "batch_015"
STATE_PATH = REPO / "tooling" / "komadeki_autopilot" / "drone_state.json"
ALL = tuple(f"B15-RISK-C{i:03d}" for i in range(1, 10))
ACCEPTED = ALL
AUTHOR_ID = "chatgpt-b15-risk-author-r1"
REVIEWER_ID = "autopilot-b15-risk-reviewer-r1"
DIRECTOR_ID = "chatgpt-primary-director-b15-r1"

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 105 or state.get("next_atomic_objective") != "MATERIALIZE_B15_ACCEPTANCE_PACKETS_9":
...
if set(rows) != set(ALL):
    raise SystemExit("unexpected B15 candidate set")
if any(rows[c]["state"] != "AI_PRE_ACCEPT" or rows[c]["permanent_question_id"].strip() for c in ALL):
    raise SystemExit("unexpected B15 pre-acceptance state")
...
if review.get("summary") != {"reviewed":9,"accept":9,"reject":0,"rework":0,"hold":0}:
    raise SystemExit("unexpected B15 review summary")
if set(review_by) != set(ALL) or any(review_by[c].get("decision") != "ACCEPT" for c in ALL):
    raise SystemExit("B15 review decision set drift")
if review.get("identity_separation") != "PASS" or review.get("author_identity_checked") != AUTHOR_ID or review.get("reviewer", {}).get("id") != REVIEWER_ID:
    raise SystemExit("B15 review identity drift")
...
if director.get("summary") != {"accept":9,"reject":0,"rework":0,"hold":0}:
    raise SystemExit("unexpected B15 Director summary")
if set(director.get("accepted_candidate_ids", [])) != set(ALL) or director.get("rejected_candidate_ids", []):
    raise SystemExit("B15 Director decision set drift")
...
if any(after[c]["state"] != "READY_FOR_ID" or after[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("B15 accepted promotion failed")
if {p.stem for p in packets.glob("*.json")} != set(ACCEPTED):
    raise SystemExit("B15 packet set drift")

with (AUTHORING / "questions.csv").open(encoding="utf-8", newline="") as h:
    if sum(1 for _ in csv.DictReader(h)) != 345:
...
state["state_epoch"] = 106
state["next_atomic_objective"] = "ALLOCATE_AND_INTEGRATE_B15_ACCEPTED_9"
STATE_PATH.write_text(json.dumps(state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
errors = validate_expansion_batch(BATCH)
if errors:
    raise SystemExit("B15 expansion validation failed: " + " | ".join(errors))
