...
EXPECTED = {cid: f"OTSU4-Q-{n:06d}" for cid, n in zip(ACCEPTED, range(7, 30))}

state = json.loads(STATE_PATH.read_text(encoding="utf-8"))
if state.get("state_epoch") != 31 or state.get("next_atomic_objective") != "ALLOCATE_AND_INTEGRATE_OTSU4_BATCH_2_ACCEPTED_23":
...
if any(before[c]["state"] != "READY_FOR_ID" or before[c]["permanent_question_id"] for c in ACCEPTED):
    raise SystemExit("accepted Otsu4 Batch 2 pre-ID state mismatch")
if any(before[c]["state"] != "AI_PRE_ACCEPT" or before[c]["permanent_question_id"] for c in REJECTED):
    raise SystemExit("rejected Otsu4 Batch 2 state mismatch")
if {p.stem for p in (BATCH / "acceptance_packets").glob("*.json")} != set(ACCEPTED):
    raise SystemExit("Otsu4 Batch 2 acceptance packet set drift")
...
if plan.mapping != EXPECTED:
    raise SystemExit(f"unexpected Otsu4 Batch 2 allocation: {plan.mapping}")
if transaction.apply() != EXPECTED:
    raise SystemExit("Otsu4 Batch 2 applied mapping drift")
...
if any(after[c]["state"] != "AI_PRE_ACCEPT" or after[c]["permanent_question_id"] for c in REJECTED):
    raise SystemExit("rejected Otsu4 Batch 2 candidate mutated")
if len(questions) != 29 or len(registry) != 29:
...
if len(verifications) != 6 or any(qid in verified_ids for qid in EXPECTED.values()):
...
state["state_epoch"] = 32
state["next_atomic_objective"] = "VERIFY_OTSU4_BATCH_2_CANONICAL_SOURCES_23"
