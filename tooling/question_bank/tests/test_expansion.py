...
class ExpansionProtocolTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.batch = Path(self.temp.name) / "batch_001"
        self.batch.mkdir()
    ...
    def test_invalid_target_decision_fails(self) -> None:
        ...
        payload["target_size_decisions"][0]["proposed_target_min"] = 30
        payload["target_size_decisions"][0]["proposed_target_max"] = 20
    ...
    def test_known_rejected_candidate_id_cannot_be_reused(self) -> None:
        path = self.batch / "batch.json"
        payload = json.loads(path.read_text(encoding="utf-8"))
        payload["planned_scope"]["known_rejected_ids"] = ["B1-C001"]
        path.write_text(json.dumps(payload), encoding="utf-8")
