...
class Eisei1B8AcceptancePacketTest(unittest.TestCase):
    ...
    def test_exact_independent_accepts_are_integrated_with_bound_packets(self) -> None:
        ...
        for candidate_id in self.mapping:
            ...
            collision = packet["evidence"]["collision"]
            self.assertTrue(collision["released_bank_checked"])
            self.assertTrue(collision["canonical_drafts_checked"])
            self.assertTrue(collision["batch_checked"])
