...
class Release188Test(unittest.TestCase):
 def test_release_is_exact_and_generated(self):
  meta=json.loads((A/"bank.json").read_text(encoding="utf-8")); self.assertEqual(CURRENT_REV,meta["bank_revision"]); self.assertEqual("2026-08-24",meta["content_as_of"])
  ...
  self.assertTrue(all(reg[i]["first_used_bank_revision"]==OLD for i in ids[:100])); self.assertTrue(all(reg[i]["first_used_bank_revision"]==EXPANSION_REV for i in ids[100:])); self.assertTrue(all(not reg[qid]["first_used_bank_revision"] for qid in reg if int(qid.rsplit("-",1)[1])>188))
  ...
  manifest=json.loads((BANK/"generated/bank_manifest.json").read_text(encoding="utf-8")); self.assertEqual(188,manifest["question_count"]); self.assertEqual(30,manifest["free_question_count"]); self.assertEqual(CURRENT_REV,manifest["bank_revision"])
