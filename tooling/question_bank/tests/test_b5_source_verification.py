...
class B5SourceVerificationTest(unittest.TestCase):
 def test_b5_is_verified_without_release_activation(self):
  ...
  self.assertTrue(set(QIDS).issubset(q)); self.assertGreaterEqual(len(q),196); self.assertTrue(all(q[i]["status"]=="draft" for i in QIDS))
  ...
  released=json.loads((A/"released_questions.json").read_text(encoding="utf-8"))["released_questions"]; self.assertEqual(188,len(released)); self.assertEqual([f"DRONE-Q-{i:06d}" for i in range(1,189)],[r["question_id"] for r in released])
  meta=json.loads((A/"bank.json").read_text(encoding="utf-8")); runtime=json.loads((BANK/meta["runtime_output"]).read_text(encoding="utf-8")); cards=[c for d in runtime["decks"] for u in d["units"] for c in u["cards"]]; self.assertEqual(188,len(cards)); self.assertEqual({f"DRONE-Q-{i:06d}" for i in range(1,189)},{c["stableId"] for c in cards})
