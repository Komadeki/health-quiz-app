...
void validateDependencyBoundary(
  Directory repositoryRoot,
  AppManifest manifest,
  ManifestValidationResult result,
) {
  if (!manifest.appDirectory.startsWith('reference_apps/')) return;
  final libDirectory = Directory(
    p.join(repositoryRoot.path, manifest.appDirectory, 'lib'),
  );
  ...
  for (final entity in libDirectory.listSync(recursive: true)) {
    ...
    if (source.contains('package:health_quiz_app/') ||
      ...
      result.error(
        'reference_app_health_dependency',
        'Reference app Dart must not import health app code.',
...
void validateQuizEngineBoundary(
  Directory repositoryRoot,
  ManifestValidationResult result,
) {
  final libDirectory = Directory(
    p.join(repositoryRoot.path, 'packages', 'quiz_engine', 'lib'),
  );
  if (!libDirectory.existsSync()) return;
  for (final entity in libDirectory.listSync(recursive: true)) {
    if (entity is! File || !entity.path.endsWith('.dart')) continue;
    final source = entity.readAsStringSync();
    if (source.contains('package:health_quiz_app/') ||
        source.contains('reference_apps/')) {
      result.error(
        'quiz_engine_app_dependency',
        'quiz_engine must not depend on an app shell.',
        p.relative(entity.path, from: repositoryRoot.path),
      );
    }
  }
